    ...
    for breaker in gateway.breakers.values():
        ...
        for transition in breaker.transition_log:
            ...
            if to_state == "open":
                ...
                recovery_times_ms.append((ts - opened_ts) * 1000.0)
                opened_ts = None
...
def _finalize_metrics(metrics: RunMetrics, gateway: ReliabilityGateway) -> RunMetrics:
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    ...
    return _finalize_metrics(metrics, gateway)
...
def run_concurrent_scenario(
    ...
    """Run one chaos scenario with concurrent request pressure."""
...
def evaluate_scenario(name: str, result: RunMetrics) -> bool:
    """Return pass/fail status using scenario-aware criteria."""

    if name == "primary_timeout_100":
        return result.fallback_success_rate >= 0.9 and result.circuit_open_count > 0
    if name == "primary_flaky_50":
        return result.availability >= 0.9 and result.fallback_successes > 0 and result.circuit_open_count > 0
    if name == "all_healthy":
        return result.availability >= 0.95 and result.error_rate <= 0.05
    if name == "all_providers_degraded":
        return result.static_fallbacks > 0 and result.circuit_open_count > 0
